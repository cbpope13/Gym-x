const Navbar = () => {
	return (
		<nav className="bg-neutral-900 flex items-center px-10 py-4 justify-between fixed w-full z-50">
			<h1 className="text-red-600 font-bold text-4xl">
				<a href="/">
					<i>GYM-X</i>
				</a>
			</h1>
			<div className="text-white text-lg flex space-x-20">
				<a href="/#home" className="hover:text-red-600 transition duration-500">
					Home
				</a>
				<a
					href="#memberships"
					className="hover:text-red-600 transition duration-500"
				>
					Memberships
				</a>
				<a
					href="#personal-training"
					className="hover:text-red-600 transition duration-500"
				>
					Personal Training
				</a>
			</div>
			<div>
				<button className="bg-red-600 rounded-sm text-white text-xl flex space-x-3 items-center px-6 py-1 hover:bg-red-700 transition duration-200">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						viewBox="0 0 24 24"
						fill="currentColor"
						className="w-4 h-4"
					>
						<path
							fillRule="evenodd"
							d="M1.5 4.5a3 3 0 013-3h1.372c.86 0 1.61.586 1.819 1.42l1.105 4.423a1.875 1.875 0 01-.694 1.955l-1.293.97c-.135.101-.164.249-.126.352a11.285 11.285 0 006.697 6.697c.103.038.25.009.352-.126l.97-1.293a1.875 1.875 0 011.955-.694l4.423 1.105c.834.209 1.42.959 1.42 1.82V19.5a3 3 0 01-3 3h-2.25C8.552 22.5 1.5 15.448 1.5 6.75V4.5z"
							clipRule="evenodd"
						/>
					</svg>
					<i>401-555-1234</i>
				</button>
			</div>
		</nav>
	);
};

export default Navbar;
